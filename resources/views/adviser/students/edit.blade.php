...
<div class="container-fluid">
    <div class="card">
        <div class="card-header">
            <h5>Edit Student: {{ $student->first_name }} {{ $student->last_name }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('adviser.students.update', $student->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('id_number') is-invalid @enderror" id="id_number" name="id_number" value="{{ old('id_number', $student->id_number) }}" required>
                        @error('id_number')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $student->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <small class="text-muted">Students are assigned to your department</small>
                </div>

                <div class="d-flex justify-content-between">
                    <a href="{{ route('adviser.students') }}" class="btn btn-secondary">Cancel</a>
